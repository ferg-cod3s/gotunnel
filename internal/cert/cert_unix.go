...
func runAsUser(name string, arg ...string) error {
	...
	// Only set credentials when running as root (dropping to a specific user).
	// On macOS, setting Credential as a non-root user fails with "operation not permitted".
	if os.Getuid() == 0 {
